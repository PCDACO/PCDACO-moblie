...
const SelectTrigger = React.forwardRef<SelectPrimitive.TriggerRef, SelectPrimitive.TriggerProps>(
  ({ className, children, ...props }, ref) => (
    <SelectPrimitive.Trigger
      ...
      <Feather name="chevron-down" size={16} className="text-foreground opacity-50" />
...
const SelectScrollUpButton = ({ className, ...props }: SelectPrimitive.ScrollUpButtonProps) => {
  ...
  return (
    <SelectPrimitive.ScrollUpButton
      ...
      {...props}>
      <Feather name="chevron-up" size={16} className="text-foreground" />
    </SelectPrimitive.ScrollUpButton>
  );
};
...
const SelectScrollDownButton = ({ className, ...props }: SelectPrimitive.ScrollDownButtonProps) => {
  ...
  return (
    <SelectPrimitive.ScrollDownButton
      className={cn('flex items-center justify-center py-1 web:cursor-default', className)}
      {...props}>
      <Feather name="chevron-down" size={16} className="text-foreground" />
    </SelectPrimitive.ScrollDownButton>
  );
};
...
const SelectItem = React.forwardRef<SelectPrimitive.ItemRef, SelectPrimitive.ItemProps>(
  ({ className, children, ...props }, ref) => (
    <SelectPrimitive.Item
      ref={ref}
      className={cn(
        'web:group native:py-2 native:pl-10 web:hover:bg-primary/50 active:bg-primary/50 relative flex w-full flex-row items-center rounded-sm py-1.5 pl-8 pr-2 active:text-white web:cursor-default web:select-none web:outline-none web:hover:text-white web:focus:bg-primary',
      ...
      <View className="native:left-3.5 native:pt-px absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
        <SelectPrimitive.ItemIndicator>
          <Feather name="check" size={16} strokeWidth={3} className="text-popover-foreground" />
        </SelectPrimitive.ItemIndicator>
      </View>
